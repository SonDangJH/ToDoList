...
export default function EditNote({currentNote, mode, noteBoardContent, actionNote, setMode, setNoteBoardContent}) {
    ...
    return (
        <div className="relative shadow-md h-[500px] lg:h-full">
            <p className="absolute z-10 top-5 left-4 text-sm text-gray-500 flex flex-row items-center gap-2"><GoClock className="w-3 h-3"/>{currentNote ? `${currentNote.date.toLocaleTimeString()} ${currentNote.date.toLocaleDateString('en-GB')}` : "Beautiful day"}</p>
            ...
            <textarea className="absolute top-0 left-0 border-[1px] rounded-md shadow w-full h-full p-4 pt-12" 
                      onChange={(e)=>{
                        setNoteBoardContent(e.target.value);
                      }}
                      value={noteBoardContent}>
            </textarea>
        </div>
    )
}